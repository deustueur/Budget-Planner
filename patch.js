...
const TAB_ISLANDS = {
  ...
  cashflow: [
    { id:'cf-main-card',       label:'Cash flow',       col:1,  row:1,  w:24, h:8 },
...
function addStaticIds() {
  ...
  if (ins) {
    wrapEl('ins-networth',  ins.querySelector('.networth-card'));
    ...
    wrapEl('ins-import',    buildInsBlock([nthInsTitle(2), document.getElementById('import-panel-local'), document.getElementById('import-panel-drive')]));
    ...
    wrapEl('ins-templates', buildInsBlock([nthInsTitle(6), document.getElementById('templates-list'), ins.querySelector('button[onclick*="saveTemplate"]')]));
    // Charts — find by content not by fragile class hunting
    const twoCol = ins.querySelector('.two-col');
    if (twoCol) wrapEl('ins-charts', buildInsBlock([nthInsTitle(5), twoCol]));
  }
}
...
function patchBank() {
  // Injects delete (✕) buttons into history bank items after each render.
  // Uses BudgetPlanner.deleteHistoryMonth for the actual deletion logic.
  ...
  function addX() {
    ...
    list.querySelectorAll('.history-bank-item').forEach(item => {
      ...
      if (!key) {
        ...
        if (span && window.BudgetPlanner.state.insightHistory) {
      ...
      btn.onclick = ev => {
        ev.stopPropagation();
        // Core deletion logic lives in html engine via port
        if (window.BudgetPlanner) window.BudgetPlanner.deleteHistoryMonth(key);
      };
      item.appendChild(btn);
    });
  }
  // Wrap via ports — not by overwriting window.renderHistoryBank directly
  window.BudgetPlanner.renderHistoryBank = function() { origRHB(); setTimeout(addX,40); };
  window.BudgetPlanner.renderInsights    = function() { origRI();  setTimeout(addX,80); };
}
...
function patchInit() {
  if (!window.BudgetPlanner) {
    console.error('[patch] window.BudgetPlanner not found. Is index.html up to date?');
    return;
  }
  injectStyles();
  loadLayout();
  ...
  patchShowTab();
  patchState();
  // Hook onAfterRender for layout lock after every recalc
  const origRender = window.BudgetPlanner.onAfterRender;
  window.BudgetPlanner.onAfterRender = function() {
    lockAllPositions();
    if (origRender) origRender();
  };
  console.log('[patch v6] ✓ loaded — engine: index.html | ui: patch.js | ports: window.BudgetPlanner');
}
